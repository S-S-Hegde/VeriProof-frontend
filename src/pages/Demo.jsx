...
const LiveChart = () => {
  ...
  useEffect(() => {
    ...
    const generate = () => {
      ...
      for (let i = 0; i <= 20; i++) {
        pts.push({
          x: i * 25,
          y: 100 + Math.sin(i * 0.5 + Date.now() * 0.001) * 40 + Math.random() * 20,
        });
      }
      return pts;
    };
    setPoints(generate());
    const interval = setInterval(() => setPoints(generate()), 2000);
  ...
  const pathD = points.length > 0
    ...
    : "";
  const areaD = pathD ? `${pathD} L500,200 L0,200 Z` : "";
  ...
  return (
    <svg viewBox="0 0 500 200" preserveAspectRatio="none" className="w-full h-full">
      ...
      {[50, 100, 150].map(y => (
        <line key={y} x1="0" y1={y} x2="500" y2={y} stroke="currentColor" className="opacity-5" strokeWidth="1" />
      ))}
      <path d={areaD} fill="url(#chartGradDemo)">
        <animate attributeName="d" values={areaD} dur="2s" />
      </path>
      <path d={pathD} fill="none" stroke="var(--color-accent)" strokeWidth="2" className="transition-all duration-1000" />
      {points.length > 0 && (
        <circle cx={points[points.length - 1]?.x} cy={points[points.length - 1]?.y} r="4" fill="var(--color-accent)">
    ...
    <div className="border border-[var(--color-border)] p-8 lg:p-10 hover:bg-[var(--color-accent)]/5 transition-all duration-700 relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-30">{num} //</span>
        </div>
...
const FeatureRow = ({ icon: Icon, label, desc, delay }) => (
  ...
  >
    ...
    <div>
      <div className="text-sm font-bold uppercase tracking-widest mb-1 group-hover:text-[var(--color-accent)] transition-colors duration-500">{label}</div>
      <div className="text-xs opacity-40 leading-relaxed max-w-sm group-hover:opacity-60 transition-opacity duration-500">{desc}</div>
    </div>
  </motion.div>
);
  ...
  return (
    <div ref={compRef} className="overflow-hidden">
      ...
      <AnimatePresence>
        {!isShutterOpen && (
          <>
            ...
            >
              <span className="text-[var(--color-bg)] text-5xl md:text-7xl font-black italic uppercase tracking-tighter">VERI</span>
            </motion.div>
            <motion.div
              initial={{ x: 0 }}
              animate={{ x: "100%" }}
            ...
            >
              <span className="text-[var(--color-accent)] text-5xl md:text-7xl font-black italic uppercase tracking-tighter">PROOF</span>
            </motion.div>
          </>
        )}
      </AnimatePresence>
      ...
      <section className="relative min-h-[100vh] flex flex-col items-center justify-center overflow-hidden -mt-28 px-6">
        ...
        >
          ...
          <div className="flex justify-center mb-10">
            <div className="flex items-center gap-3 px-5 py-2.5 border border-[var(--color-border)] backdrop-blur-md rounded-full">
              ...
              <span className="text-[10px] font-mono uppercase tracking-[0.35em] opacity-70">Core_Verified_Archive</span>
            </div>
          </div>
          ...
          <TextReveal delay={0.4}>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black italic uppercase tracking-tighter leading-[0.85] mb-6">
              The <span style={{ WebkitTextStroke: "2px var(--color-text)", WebkitTextFillColor: "transparent" }}>Evidence</span><br/>
              <span className="text-[var(--color-accent)] not-italic">Protocol.</span>
            </h1>
          </TextReveal>
        ...
        >
          <span className="text-[9px] font-mono uppercase tracking-[0.4em] opacity-30">Explore</span>
          <motion.div
            animate={{ y: [0, 8, 0] }}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-20 max-w-3xl">
            ...
            >
              ...
              <span className="text-[10px] font-mono uppercase tracking-[0.4em]">How_It_Works</span>
            </motion.div>
            <TextReveal>
              <h2 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter leading-[0.85]">
                Three Steps To<br /><span className="text-[var(--color-accent)] not-italic">Verified Truth.</span>
              </h2>
            </TextReveal>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1">
            <ProcessStep num="01" icon={Fingerprint} title="Connect Identity" desc="Link your GitHub, GitLab, and other code repositories. Our system begins mapping your digital fingerprint across all connected nodes." delay={0} />
            <ProcessStep num="02" icon={Scan} title="Forensic Analysis" desc="Neural scanning verifies every commit, pull request, and code contribution. Cross-referencing billions of lines to ensure 100% authenticity." delay={0.15} />
            <ProcessStep num="03" icon={Shield} title="Issue Proof" desc="Receive a cryptographically signed VeriProof certificate — an immutable record of your verified skills that recruiters can trust." delay={0.3} />
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
            ...
            <div>
              ...
              >
                ...
                <span className="text-[10px] font-mono uppercase tracking-[0.4em]">Evidence_Protocol</span>
              </motion.div>
              <TextReveal>
                <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-[0.85] mb-10">
                  The Evidence<br /><span className="text-[var(--color-accent)] not-italic">Architecture.</span>
                </h2>
              </TextReveal>

              <FeatureRow icon={Fingerprint} label="Biometric Verification" desc="Hash-based identity validation across distributed nodes with zero-knowledge proofs." delay={0} />
              <FeatureRow icon={Globe} label="Global Sync" desc="Real-time synchronization with primary talent repositories across 47 jurisdictions." delay={0.1} />
              <FeatureRow icon={Layers} label="Neural Archive" desc="Permanent, immutable record of skill manifestations encoded in append-only ledgers." delay={0.2} />
              <FeatureRow icon={Eye} label="Transparency Layer" desc="Every verification decision is auditable. Full traceability from raw commit to final proof." delay={0.3} />
            </div>
            ...
            >
              <div className="border border-[var(--color-border)] bg-black/5 dark:bg-white/5 backdrop-blur-xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)] bg-white/5 dark:bg-black/10">
                  <span className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-50">Live_Network_Load</span>
                ...
                <div className="grid grid-cols-3 border-t border-[var(--color-border)]">
                  {METRICS.map((m, i) => (
                    <div key={i} className={`p-5 text-center ${i < 2 ? "border-r border-[var(--color-border)]" : ""}`}>
                      <div className={`text-xl md:text-2xl font-black italic tracking-tighter ${m.color}`}>{m.value}</div>
                      <div className="text-[9px] font-mono uppercase tracking-[0.2em] opacity-40 mt-1">{m.label}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <div className="text-xs font-bold uppercase tracking-widest mb-1">Verification Guarantee</div>
                  <div className="text-[10px] opacity-40 leading-relaxed">
                    Every proof generated by VeriProof is cryptographically signed and can be independently verified by any third party without requiring access to our systems.
                  </div>
                </div>
              </motion.div>
            </motion.div>
      ...
      {/* ═══════════════════ CTA ═══════════════════ */}
      <section id="demo-cta-new" className="relative py-32 lg:py-44 px-6 border-t border-[var(--color-border)]">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <TextReveal>
            <h2 className="demo-cta-title text-5xl md:text-7xl lg:text-8xl font-black italic uppercase tracking-tighter leading-[0.85]">
              Start_The<br /><span className="text-[var(--color-accent)] not-italic">Audit.</span>
            </h2>
          </TextReveal>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
          <div>
            ...
            <p className="text-xs opacity-40 leading-relaxed max-w-xs">
              A high-fidelity truth layer for professional identities. Powered by Veri-Protocol v4.2.
            </p>
          </div>
          <div>
            <h5 className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-30 mb-4">Section_01</h5>
            <ul className="space-y-2">
              {["Terminal", "Evidence", "Verification", "Security"].map(item => (
                <li key={item} className="text-xs opacity-50 hover:opacity-100 hover:text-[var(--color-accent)] transition-all cursor-pointer">{item}</li>
              ))}
            </ul>
          </div>
          <div>
            <h5 className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-30 mb-4">Section_02</h5>
            <ul className="space-y-2">
