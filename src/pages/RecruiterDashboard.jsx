    ...
    <PageTransition>
      <div className="md:flex md:items-end md:justify-between mb-12 border-b border-[var(--color-border)] pb-8">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-mono tracking-[0.4em] uppercase opacity-40 mb-2 flex items-center gap-2">
        ...
        <div className="mt-8 flex md:mt-0 md:ml-4">
          <Link
            to="/discover"
            className="px-8 py-3 bg-[var(--color-accent)] text-[var(--color-bg)] font-bold tracking-[0.2em] uppercase text-sm flex items-center gap-2 hover:opacity-90 transition-all shadow-[0_0_20px_var(--color-accent)]/20"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="glass-card p-6 border-l-4 border-[var(--color-accent)]">
          <p className="text-sm tracking-[0.2em] uppercase opacity-50 mb-1">Authenticated_Talent</p>
          <p className="text-3xl font-bold h1">1,204</p>
        </div>
        <div className="glass-card p-6 border-l-4 border-[var(--color-accent)]/30">
          <p className="text-sm tracking-[0.2em] uppercase opacity-50 mb-1">Pending_Verifications</p>
        ...
        <div className="glass-card p-6 border-l-4 border-[var(--color-accent)]/10">
          <p className="text-sm tracking-[0.2em] uppercase opacity-50 mb-1">High_Integrity_Matches</p>
              ...
              <div key={student._id} className="glass-card flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 group hover:border-[var(--color-accent)] transition-all">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm tracking-[0.2em] uppercase opacity-40 font-mono">GH: @{student.githubUsername}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-8">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="flex items-center gap-2 text-sm tracking-[0.2em] uppercase opacity-60 hover:opacity-100 transition-opacity underline decoration-[var(--color-accent)] underline-offset-4"
                  >
                    Examine_Docs <ExternalLink className="w-3 h-3" />
                  </a>
                  <button 
                    onClick={() => handleVerifyResume(student._id)}
                    className="px-6 py-2 bg-[var(--color-accent)] text-[var(--color-bg)] text-sm tracking-[0.2em] uppercase font-bold hover:opacity-90 transition-all flex items-center gap-2"
                  >
      ...
      {loading ? (
        <div className="flex flex-col items-center py-20 opacity-40">
           ...
           <p className="text-sm font-mono tracking-widest uppercase">Fetching_Archive_Data...</p>
        </div>
      ) : projects.length === 0 ? (
        <div className="text-center glass-card py-20 border opacity-40">
          <h3 className="text-sm uppercase tracking-[0.3em]">
            ...
            >
              <div className="p-8 flex-grow">
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-sm font-mono tracking-widest uppercase opacity-40">
                      {project.user.name}
                    </h3>
                    <p className="text-sm font-mono uppercase tracking-tighter mt-1 text-[var(--color-accent)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.slice(0, 3).map((tech, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center px-2 py-1 text-xs tracking-[0.2em] uppercase font-mono border border-[var(--color-border)] text-[var(--color-muted)]"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              <div className="px-8 py-5 flex items-center justify-between border-t border-[var(--color-border)] bg-black/5">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm uppercase tracking-[0.2em] font-bold border-b border-transparent hover:border-[var(--color-accent)] text-[var(--color-accent)] transition-all"
                >
                  Source_Archive
                </a>
                <span className={`inline-flex items-center px-2 py-1 text-xs tracking-[0.2em] uppercase font-mono border ${project.isVerified ? "border-[var(--color-accent)] text-[var(--color-accent)]" : "opacity-40"}`}>
