...
const calculateVerifiedScore = (skill) => {
  ...
  if (Array.isArray(evidenceList) && evidenceList.length > 0) {
    for (const item of evidenceList) {
      ...
      const validScore = isNaN(scoreVal) ? 100 : Math.min(100, Math.max(0, scoreVal));
  ...
  if (totalAvailableWeight === 0) {
    ...
    return isNaN(fallback) ? 0 : Math.min(100, Math.max(0, Math.round(fallback)));
  }
...
const getSkillCategory = (skill) => {
  ...
  if (/\b(aws|azure|gcp|google\s*cloud|firebase|heroku|vercel|cloud)\b/i.test(name)) {
    return "Cloud";
  }
...
const getConnectedNodeIds = (selectedSkill, visibleTree) => {
  ...
  const selectedId = (selectedSkill.id || selectedSkill.name || "").toLowerCase();
  connected.add(selectedId);

  // Add prerequisites (parents)
  if (Array.isArray(selectedSkill.prerequisites)) {
    selectedSkill.prerequisites.forEach((p) => connected.add((p || "").toLowerCase()));
  }
    ...
    for (const cat of visibleTree.categories) {
      if (cat.skills) {
        for (const sk of cat.skills) {
          ...
          const prereqs = (sk.prerequisites || []).map((p) => (p || "").toLowerCase());
...
const ROADMAP_CATEGORIES_CONFIG = [
  { id: "programming", name: "PROGRAMMING LANGUAGES", icon: Code2, color: "#3b82f6", total: 14, defaultUnlocked: 7 },
  { id: "frontend", name: "FRONTEND", icon: Monitor, color: "#22c55e", total: 14, defaultUnlocked: 6 },
  { id: "backend", name: "BACKEND", icon: Server, color: "#f59e0b", total: 15, defaultUnlocked: 5 },
  { id: "database", name: "DATABASE", icon: Database, color: "#14b8a6", total: 12, defaultUnlocked: 4 },
  { id: "devops", name: "DEVOPS", icon: GitBranch, color: "#a855f7", total: 13, defaultUnlocked: 5 },
  { id: "ai-ml", name: "AI / ML", icon: Brain, color: "#ec4899", total: 10, defaultUnlocked: 3 },
  { id: "cloud", name: "CLOUD", icon: Cloud, color: "#06b6d4", total: 9, defaultUnlocked: 3 },
  { id: "cybersecurity", name: "CYBERSECURITY", icon: Shield, color: "#ef4444", total: 11, defaultUnlocked: 2 },
  { id: "fullstack", name: "FULL STACK", icon: Network, color: "#6366f1", total: 16, defaultUnlocked: 6 },
];

const ROADMAP_NODES = [
  // Tier 1: Programming Languages
  { id: "cat-programming", type: "category", label: "PROGRAMMING LANGUAGES", color: "#3b82f6", x: 520, y: 35 },
  { id: "js", name: "JavaScript", short: "JS", categoryId: "programming", parentId: "cat-programming", x: 520, y: 115, icon: Code2 },
  { id: "ts", name: "TypeScript", short: "TS", categoryId: "programming", parentId: "js", x: 370, y: 225, icon: Code2 },
  { id: "python", name: "Python", short: "PY", categoryId: "programming", parentId: "js", x: 520, y: 225, icon: Code2 },
  { id: "html", name: "HTML/CSS", short: "HTML", categoryId: "programming", parentId: "js", x: 670, y: 225, icon: Code2 },

  // Tier 2: Frontend & Backend
  { id: "cat-frontend", type: "category", label: "FRONTEND", color: "#22c55e", parentId: "ts", x: 270, y: 315 },
  { id: "react", name: "React", short: "⚛️", categoryId: "frontend", parentId: "cat-frontend", x: 170, y: 405, icon: Monitor },
  { id: "nextjs", name: "Next.js", short: "N", categoryId: "frontend", parentId: "cat-frontend", x: 270, y: 405, icon: Monitor },
  { id: "frontend-testing", name: "Frontend Testing", short: "🧪", categoryId: "frontend", parentId: "cat-frontend", x: 370, y: 405, icon: Monitor },

  { id: "cat-backend", type: "category", label: "BACKEND", color: "#f59e0b", parentId: "python", x: 770, y: 315 },
  { id: "nodejs", name: "Node.js", short: "🟢", categoryId: "backend", parentId: "cat-backend", x: 620, y: 405, icon: Server },
  { id: "express", name: "Express", short: "ex", categoryId: "backend", parentId: "cat-backend", x: 720, y: 405, icon: Server },
  { id: "rest-api", name: "REST API", short: "{--}", categoryId: "backend", parentId: "cat-backend", x: 820, y: 405, icon: Server },
  { id: "authentication", name: "Authentication", short: "🔒", categoryId: "backend", parentId: "cat-backend", x: 920, y: 405, icon: Server },

  // Tier 3: Database, DevOps & Cloud
  { id: "cat-database", type: "category", label: "DATABASE", color: "#14b8a6", parentId: "react", x: 220, y: 495 },
  { id: "mongodb", name: "MongoDB", short: "🍃", categoryId: "database", parentId: "cat-database", x: 170, y: 585, icon: Database },
  { id: "mongoose", name: "Mongoose", short: "M", categoryId: "database", parentId: "cat-database", x: 270, y: 585, icon: Database },

  { id: "cat-devops", type: "category", label: "DEVOPS", color: "#a855f7", parentId: "nodejs", x: 530, y: 495 },
  { id: "git", name: "Git/GitHub", short: "🐙", categoryId: "devops", parentId: "cat-devops", x: 430, y: 585, icon: GitBranch },
  { id: "docker", name: "Docker", short: "🐳", categoryId: "devops", parentId: "cat-devops", x: 530, y: 585, icon: GitBranch },
  { id: "cicd", name: "CI/CD", short: "∞", categoryId: "devops", parentId: "cat-devops", x: 630, y: 585, icon: GitBranch },

  { id: "cat-cloud", type: "category", label: "CLOUD", color: "#06b6d4", parentId: "authentication", x: 840, y: 495 },
  { id: "aws", name: "AWS", short: "☁️", categoryId: "cloud", parentId: "cat-cloud", x: 740, y: 585, icon: Cloud },
  { id: "cloud-docker", name: "Docker", short: "🐳", categoryId: "cloud", parentId: "cat-cloud", x: 840, y: 585, icon: Cloud },
  { id: "kubernetes", name: "Kubernetes", short: "☸️", categoryId: "cloud", parentId: "cat-cloud", x: 940, y: 585, icon: Cloud },

  // Tier 4: AI/ML, Cybersecurity & Full Stack
  { id: "cat-aiml", type: "category", label: "AI / ML", color: "#ec4899", parentId: "mongodb", x: 270, y: 675 },
  { id: "ml", name: "Machine Learning", short: "🧠", categoryId: "ai-ml", parentId: "cat-aiml", x: 210, y: 765, icon: Brain },
  { id: "dl", name: "Deep Learning", short: "🕸️", categoryId: "ai-ml", parentId: "cat-aiml", x: 330, y: 765, icon: Brain },

  { id: "cat-cybersecurity", type: "category", label: "CYBERSECURITY", color: "#ef4444", parentId: "docker", x: 610, y: 675 },
  { id: "owasp", name: "OWASP Top 10", short: "🛡️", categoryId: "cybersecurity", parentId: "cat-cybersecurity", x: 540, y: 765, icon: Shield },
  { id: "network-security", name: "Network Security", short: "🌐", categoryId: "cybersecurity", parentId: "cat-cybersecurity", x: 680, y: 765, icon: Shield },

  { id: "cat-fullstack", type: "category", label: "FULL STACK", color: "#6366f1", parentId: "kubernetes", x: 890, y: 675 },
  { id: "mern", name: "MERN Stack", short: "📚", categoryId: "fullstack", parentId: "cat-fullstack", x: 820, y: 765, icon: Network },
  { id: "system-design", name: "System Design", short: "🔲", categoryId: "fullstack", parentId: "cat-fullstack", x: 960, y: 765, icon: Network },
];
...
const ROADMAP_EDGES = [
  ...
  { from: "authentication", to: "cat-cloud", color: "#06b6d4", style: "dashed" },
  ...
  { from: "docker", to: "cat-cybersecurity", color: "#ef4444", style: "dashed" },
  { from: "cat-cybersecurity", to: "owasp", color: "#ef4444" },
  { from: "cat-cybersecurity", to: "network-security", color: "#ef4444" },
  { from: "kubernetes", to: "cat-fullstack", color: "#6366f1", style: "dashed" },
...
const VerifiedSkillCard = ({ skill, rank, onClick }) => {
  ...
  const getRankBadge = (r) => {
    if (r === 1) return { label: "#1 🥇", style: "bg-amber-500/15 text-amber-400 border-amber-500/30" };
    if (r === 2) return { label: "#2 🥈", style: "bg-slate-300/15 text-slate-300 border-slate-400/30" };
    if (r === 3) return { label: "#3 🥉", style: "bg-amber-700/15 text-amber-500 border-amber-600/30" };
    return { label: `#${r}`, style: "bg-[var(--color-bg-sunken)] text-[var(--color-muted)] border-[var(--color-border)]" };
  };
...
const SkillTreePage = () => {
  ...
  const handleGraphMouseMove = (e) => {
    ...
    const containerNode = e.target.closest(".group") || e.target.closest(".absolute");
    ...
    const matchedSkill = findSkillFromGraphNode(identifier, visibleTree, visibleProgress);
    ...
    x = Math.max(tooltipWidth / 2 + 16, Math.min(viewportWidth - tooltipWidth / 2 - 16, x));
  ...
  useEffect(() => {
    ...
    nodeElements.forEach((el) => {
      ...
      const matchedSkill = findSkillFromGraphNode(identifier, visibleTree, visibleProgress);
      const matchedId = (matchedSkill?.id || matchedSkill?.name || identifier).toLowerCase();

      const isSelected = matchedId === selectedId || identifier.includes(selectedId);
      ...
      if (isSelected && btn) {
        ...
        btn.style.boxShadow = "0 0 30px rgba(59, 130, 246, 0.9), 0 0 45px rgba(16, 185, 129, 0.7)";
            ...
            <>
              <div className="flex items-center gap-3 mb-6">
                <span className="text-[10px] font-mono uppercase tracking-wider text-[var(--color-muted)]">
                  ...
                  {verifiedCount} verified • {inProgressCount} in progress across {categorySections.length} categor{categorySections.length !== 1 ? "ies" : "y"}
                </span>
              </div>
          ...
          <aside className="w-full lg:w-72 shrink-0 space-y-2.5">
            ...
            {ROADMAP_CATEGORIES_CONFIG.map((cat) => {
              ...
              const progressPct =
                catSkills.length > 0
                  ? Math.round(
                      catSkills.reduce((sum, s) => sum + calculateVerifiedScore(s), 0) /
                        catSkills.length
                    )
          ...
          <div className="flex-1 flex flex-col justify-between rounded-2xl border border-[var(--color-border)] bg-[#070b14] overflow-hidden shadow-2xl vp-graph-wrapper relative min-h-[860px]">
            ...
            >
              <div
                ...
                style={{ width: "1080px", height: "820px", transform: `scale(${zoom})` }}
              >
                {/* SVG Connections */}
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
                  {ROADMAP_EDGES.map((edge) => {
                    ...
                    const curve = buildRoadmapCurve(fromNode.x, fromNode.y + 20, toNode.x, toNode.y - 15);
                    const isSelected = selectedSkill && (selectedSkill.id === fromNode.id || selectedSkill.id === toNode.id);
                    ...
                    return (
                      <path
                        ...
                        d={curve}
                        fill="none"
                        stroke={isSelected ? "#3b82f6" : edge.color || "#334155"}
                ...
                {ROADMAP_NODES.map((node) => {
                  ...
                  const score = matchedSkill
                    ...
                    : node.id === "js" || node.id === "react" ? 92
                    : node.id === "ts" || node.id === "express" || node.id === "mongodb" ? 86
                    : node.id === "python" ? 90
                    : node.id === "nodejs" || node.id === "mern" ? 89
                    : node.id === "nextjs" ? 78
                    : 70;

                  const levelObj = getSkillLevel(score);
                  const profColor = getProficiencyColor(score);
                  const isSelected = selectedSkill && (selectedSkill.id === matchedSkill?.id || selectedSkill.name === node.name);
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        onClick={() => setSelectedSkill(matchedSkill || { name: node.name, id: node.id })}
                      ...
                      <div className="mt-2 text-center w-28 -translate-x-8 pointer-events-none">
                        ...
                        <p className="text-[10px] font-mono font-bold mt-0.5">
                          ...
                          <span className="text-slate-400">{levelObj.label}</span>
                        </p>
                      </div>
                    </div>
                  );
              ...
              <AnimatePresence>
                {hoveredGraphSkill && (
                  <motion.div
                    initial={{ opacity: 0, scale: 0.95, y: tooltipPos.placement === "top" ? 6 : -6 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.95, y: tooltipPos.placement === "top" ? 6 : -6 }}
                    ...
                    <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-slate-800">
                      ...
                      <span
                        className={`inline-flex items-center px-1.5 py-0.2 rounded text-[8px] font-bold uppercase tracking-wider border shrink-0 ${
                          getSkillLevel(calculateVerifiedScore(hoveredGraphSkill)).style
                        }`}
                      >
                        {getSkillLevel(calculateVerifiedScore(hoveredGraphSkill)).label}
                      </span>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="space-y-1">
                        {["github", "assessment", "resume"].map((type) => {
                          const hasEvidence = (hoveredGraphSkill.evidence || []).some(
                            (e) =>
                              (e.type || "").toLowerCase() === type ||
                              (type === "github" && (e.type || "").toLowerCase() === "project")
                          );
                          const labels = { github: "GitHub", assessment: "Assessment", resume: "Resume" };
                          return (
                            <div key={type} className="flex items-center gap-1.5 text-[10px]">
                              <span className={hasEvidence ? "text-emerald-400 font-bold" : "text-slate-600"}>
                                {hasEvidence ? "✓" : "○"}
                              </span>
                              <span className={hasEvidence ? "text-slate-200 font-medium" : "text-slate-500"}>
                                {labels[type]}
                              </span>
                            </div>
                          );
            ...
            <div className="p-4 border-t border-slate-800/80 bg-slate-950/80 backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-4 z-20">
              <div className="flex flex-wrap items-center gap-5 text-[11px] font-mono">
                ...
                <div className="flex items-center gap-3 border-l border-slate-800 pl-4">
                  <span className="flex items-center gap-1 text-emerald-400 font-bold">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> High (70%+)
                  </span>
                  <span className="flex items-center gap-1 text-amber-400 font-bold">
                    <span className="w-2 h-2 rounded-full bg-amber-500 inline-block" /> Medium (40-69%)
                  </span>
                  <span className="flex items-center gap-1 text-red-400 font-bold">
                    <span className="w-2 h-2 rounded-full bg-red-500 inline-block" /> Low (&lt;40%)
                  </span>
              ...
              <div className="flex items-center gap-2 text-[10px] font-mono text-[var(--color-muted)] border-t md:border-t-0 md:border-l border-slate-800 pt-2 md:pt-0 md:pl-4">
                ...
                <span>Scores calculated from Technical Assessment (50%), GitHub (35%), and Resume (15%).</span>
              </div>
            </div>
          </div>
        </motion.div>
