...
const Login = () => {
  ...
  return (
    <div className={`min-h-[90vh] flex items-center justify-center p-4 transition-colors duration-700`}>
          ...
          <div className="relative z-10">
            <div className="flex space-x-4 mb-8">
              ...
              >
                ...
                <span className="text-[10px] uppercase tracking-widest font-bold">The Candidate</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="text-[10px] uppercase tracking-widest font-bold">The Investigator</span>
              </button>
            </div>
        ...
        <div className="p-12 flex flex-col justify-center">
          <div className="mb-10 lg:hidden">
             <div className="flex justify-center space-x-4 mb-8">
                <button onClick={() => setRole("student")} className={`text-[10px] tracking-widest uppercase pb-2 border-b-2 transition-all ${role === "student" ? "border-blue-600 text-blue-600" : "border-transparent opacity-50"}`}>Student</button>
                <button onClick={() => setRole("recruiter")} className={`text-[10px] tracking-widest uppercase pb-2 border-b-2 transition-all ${role === "recruiter" ? "border-amber-500 text-amber-500" : "border-transparent opacity-50"}`}>Recruiter</button>
             </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-1">
              <label className="text-[10px] uppercase tracking-widest font-bold opacity-60 ml-1">Archive ID (Email)</label>
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase tracking-widest font-bold opacity-60 ml-1">Access Key (Password)</label>
              <div className={`flex items-center space-x-3 p-4 border transition-all duration-500 ${
                isDarkMode ? "bg-slate-900 border-amber-500/20 focus-within:border-amber-500" : "bg-slate-50 border-slate-200 focus-within:border-blue-600"
              }`}>
            ...
            <AnimatePresence>
              {error && (
                <motion.div 
                  ...
                  className="p-4 bg-red-500/10 border border-red-500/50 text-red-500 text-[10px] uppercase tracking-widest text-center"
                >
                  {error}
                </motion.div>
              )}
          ...
          <div className="mt-12 pt-8 border-t border-current opacity-10 flex justify-between items-center">
             <Link to="/register" className="text-[10px] uppercase tracking-widest hover:opacity-100 opacity-60 transition-opacity">Enlist for Archive Access</Link>
             <span className="text-[10px] uppercase tracking-widest opacity-30 font-mono">v1.0.4-forensic</span>
          </div>
        </div>
      </div>
    </div>
