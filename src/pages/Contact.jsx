...
import { Mail, MessageSquare, MapPin, Clock, ArrowRight, Send, Loader2 } from "lucide-react";
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    // Simulate send — wire to your backend /api/contact later
  ...
  const INFO = [
    ...
    { icon: MessageSquare, label: "Live Support", value: "Available 09:00 – 18:00 IST" },
    { icon: MapPin, label: "Node Location", value: "Bengaluru, Karnataka, India" },
      ...
      <section className="relative py-24 lg:py-36 px-6 border-b border-[var(--color-border)]">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.4em]">Contact_Protocol</span>
          </motion.div>
          <TextReveal>
            <h1 className="text-5xl md:text-7xl lg:text-9xl font-black italic uppercase tracking-tighter leading-[0.85]">
              Get In<br /><span className="text-[var(--color-accent)] not-italic">Touch.</span>
            </h1>
          </TextReveal>
          <motion.p
          ...
          >
            Have a question about VeriProof, want to report a bug, or explore an enterprise partnership? 
      ...
      <section className="py-20 lg:py-32 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-1">
          ...
          <div className="border border-[var(--color-border)] p-10 lg:p-16">
            <h2 className="text-2xl font-black uppercase tracking-tighter mb-10">Contact Info</h2>
            <div className="space-y-0">
              {INFO.map((item, i) => (
                ...
                >
                  ...
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-[0.3em] opacity-30 mb-1">{item.label}</div>
          ...
          >
            {sent ? (
              <div className="h-full flex flex-col items-center justify-center text-center gap-6">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tighter">Transmission Received</h3>
                <p className="text-sm opacity-50 max-w-sm">We've logged your message and will respond to your comms address within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                <h2 className="text-2xl font-black uppercase tracking-tighter mb-8">Send Message</h2>
                {[
                  { key: "name", label: "Full Name", type: "text", placeholder: "JOHN DOE" },
                  { key: "email", label: "Email Address", type: "email", placeholder: "NAME@DOMAIN.COM" },
                  { key: "subject", label: "Subject", type: "text", placeholder: "PARTNERSHIP / BUG REPORT / GENERAL" },
                  ...
                  <div key={key} className="relative group">
                    ...
                    />
                    <label className="absolute -top-3 left-4 px-2 text-[10px] uppercase font-bold tracking-[0.2em] bg-[var(--color-bg)]">{label}</label>
                  </div>
                ))}
                <div className="relative group">
                  ...
                  />
                  <label className="absolute -top-3 left-4 px-2 text-[10px] uppercase font-bold tracking-[0.2em] bg-[var(--color-bg)]">Message</label>
                </div>
                <button
                  type="submit"
                  disabled={loading}
                ...
                >
                  {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                    <><span>Transmit Message</span><ArrowRight className="w-4 h-4" /></>
                  )}
                </button>
              </form>
            )}
