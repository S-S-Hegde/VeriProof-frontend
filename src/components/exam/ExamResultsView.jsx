...
import { CheckCircle2, XCircle, RotateCcw, Award, BarChart3, Home } from "lucide-react";
...
const ExamResultsView = ({ result, candidateName, onReset }) => {
  ...
  const isPassed = result.status === "Passed" || result.status === "Pass" || result.score >= 70;
  const scorePercent = result.score !== undefined ? result.score : Math.round(((result.correctAnswers || 0) / (result.totalQuestions || 30)) * 100);
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 px-4">
      <div className="glass-card rounded-2xl p-8 text-center mb-6 shadow-2xl border border-slate-800">
        ...
        <h2 className="text-3xl font-extrabold text-white mb-1">Assessment Evaluation Result</h2>
        <p className="text-slate-400 text-sm">Candidate: {candidateName || "Candidate"}</p>
        ...
        <div className="my-6 py-6 rounded-xl bg-slate-900/60 border border-slate-800 max-w-xs mx-auto shadow-inner">
          ...
          <h3 className={`text-5xl font-black my-1 ${isPassed ? "text-blue-400" : "text-amber-400"}`}>
            {scorePercent}%
          </h3>
          <p className="text-xs text-slate-400 font-semibold">
            {result.correctAnswers || 0} / {result.totalQuestions || 30} Correct Answers
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 max-w-md mx-auto text-xs">
          <div className="p-3 rounded-lg bg-slate-900/40 border border-slate-800">
            ...
            <strong className="text-slate-200 text-sm font-bold">{result.answeredQuestions || result.totalQuestions || 30}</strong>
          </div>
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
            <span className="text-emerald-400 block">Correct</span>
            <strong className="text-emerald-300 text-sm font-bold">{result.correctAnswers || 0}</strong>
          </div>
      ...
      {result.certificate && (
        <div className="glass-card rounded-2xl p-6 mb-6 border border-emerald-500/40 bg-emerald-500/5 shadow-xl flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h4 className="text-sm font-bold text-white">{result.certificate.title}</h4>
              <p className="text-xs text-emerald-400 font-mono mt-0.5">Credential ID: {result.certificate.credentialId}</p>
            </div>
          </div>
          <span className="px-3 py-1 rounded bg-emerald-500/20 text-emerald-300 text-xs font-semibold">Verified</span>
        </div>
      )}
