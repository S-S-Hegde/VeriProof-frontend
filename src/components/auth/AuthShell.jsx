...
const AuthShell = ({ children, role = "student", mode = "login", step = 1 }) => {
  return (
    <div className="min-h-[90vh] w-full flex items-center justify-center p-2 sm:p-4 lg:p-6">
      <div className="w-full max-w-6xl rounded-3xl overflow-hidden border border-slate-300/80 dark:border-cyan-500/20 shadow-[0_16px_48px_rgba(0,0,0,0.45)] bg-white/70 dark:bg-[#070a14]/60 backdrop-blur-2xl grid grid-cols-1 lg:grid-cols-12 min-h-[680px] transition-colors duration-300">
        {/* Left Telemetry Column (5 Columns on Desktop) */}
        <div className="lg:col-span-5 hidden lg:block relative border-r border-slate-200/80 dark:border-white/5">
          <VerificationTelemetrySidebar role={role} mode={mode} step={step} />
        </div>

        {/* Right Terminal Deck Column (7 Columns on Desktop) */}
        <div className="lg:col-span-7 flex flex-col items-center justify-center p-6 sm:p-10 lg:p-14 relative bg-slate-50/40 dark:bg-[#090d1b]/40 backdrop-blur-xl overflow-y-auto transition-colors duration-300">
